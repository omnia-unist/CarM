...
class DenseNet(nn.Module):
    ...
    """
    def __init__(self, small_inputs=True, efficient=True):
        super(DenseNet, self).__init__()
        
        depth = 100
        width = 12
        
        width = width # opt.width -> growth_rate
        if (depth - 4) % 3: raise Exception('Depth should be of the form 3n+4')
        n = (depth - 4) // 6
        block_config = (n, n, n)
        compression = 0.5
        assert 0 < compression <= 1, 'compression of densenet should be between 0 and 1'
        num_init_features = width*2
        bn_size = 4
        drop_rate = 0
