...
class BiC_no_distill(Base):
    def __init__(self, model, opt_name, lr, lr_schedule, lr_decay, device, num_epochs, swap,
        ...
        if 'distill_only' in kwargs:
            self.distill_only = kwargs['distill_only']
        else:
            self.distill_only = False
        
        
        print("======================== DISTILL ONLY : ",self.distill_only)
        ...
        print("======================== ALPHA SET : ",self.alpha_set)
    ...
    def before_train(self, task_id):
        ...
        replay_size = len(self.replay_dataset)
        print("length of replay dataset : ", replay_size)
        
        self.stayed_iter = [0] * replay_size
        self.num_called = [0] * replay_size
        ...
        for epoch in range(self.num_epochs):
            ...
            for i, (idxs, inputs, targets) in enumerate(self.cl_dataloader):
                ...
                if self.old_model is not None:
                    ...
                    if self.distill_only == True:
                        self.alpha = 0.5
                        print(f"Distill only... alpha will be {self.alpha}")
                    if self.alpha_set > -1:
                        self.alpha = self.alpha_set
                        print(f"Alpha will be {self.alpha}")
            ...
            if epoch % 10 == 0 and epoch > 0:
                ...
                avg_top1_acc, task_top1_acc, avg_top5_acc, task_top5_acc = self.eval_task(get_entropy=False)
                print("\n\n============ ACC - INTERMEDIATE ============")
                print("task_accuracy : ", task_top1_acc)
                print("task_top5_accuracy : ", task_top5_acc)
